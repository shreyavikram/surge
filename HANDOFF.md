...
- Live: https://greenfield-w77m.onrender.com, deployed from `main` of https://github.com/shreyavikram/surge (Render service `srv-daeo1u6q1p3s73a6454g`, named greenfield; the old `srv-daebn5ht0dsc739jbsmg` at surge-bafg.onrender.com is renamed greenfield-old and can be deleted once nobody uses the old link). **Always push `main` (`git push origin HEAD`) and then trigger a deploy** with `POST https://api.render.com/v1/services/srv-daeo1u6q1p3s73a6454g/deploys` using `RENDER_API_KEY` from `.env` (the service says autoDeploy=yes but Render's GitHub app is not installed on the repo, so pushes never deploy on their own; the new service missed three pushes on 2026-09-06 until triggered); Render's GitHub sync lags a push by ~30 s, and a deploy triggered too early rebuilds the previous commit. Poll `/deploys?limit=1` until `live`.
- Marketing site: https://greenfield-site.onrender.com, a Render **static site** (`srv-daer2ldbedkc73ecsuv0`, named greenfield-site) publishing `apps/site` from `main` with no build step. Same deploy caveat as the terminal — push `main`, then `POST https://api.render.com/v1/services/srv-daer2ldbedkc73ecsuv0/deploys` and poll `/deploys?limit=1` until `live`. The two maps it draws are committed (`apps/site/assets/maps.js`); regenerate them with `npm run graphics -w @surge/site` after changing commodity, origin-share or district data, and `apps/site/test/claims.test.ts` fails if any count printed on the page drifts from the config it came from.
...
- Deploy: `Dockerfile` + `render.yaml` run the Hono server with the built client in one container. Live at https://greenfield-w77m.onrender.com (Render service `srv-daebn5ht0dsc739jbsmg`, owner `tea-dae73kmq1p3s7387gmdg`, auto-deploys from `main` of the public repo https://github.com/shreyavikram/surge). Env vars are set in Render. Vite dev proxies `/api` to `localhost:8787`.

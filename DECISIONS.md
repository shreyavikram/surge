...
24. **The marketing site is a separate, framework-free static app, built on the terminal's own surface.**
    `apps/site` is plain HTML/CSS/JS deployed as its own Render service, so the terminal's live URL is
    untouched by it. It is terminal-first: the page uses the app's dark token values (`apps/web/src/theme.css`),
    opens with a tape of the readouts the terminal was serving, and shows the product through real
    screenshots of the running app rather than reproductions of its panels. The deck (Greenfield.pptx)
    supplies the identity that survives the dark treatment — the serif headline with a green italic second
    phrase, the greens, the Courier-lineage mono on every label and number — and the coverage and company
    sections keep the deck's paper ground as the document moment of the page. Structure follows a
    commodity-intelligence solution page: hero, hard stat bar, product blocks with full-width screenshots,
    method, itemized coverage taxonomy, business case. The two maps are generated from the terminal's own
    geodata and cited sources — the world map coloured by each country's share of US food imports (Census
    customs value), the district map by each district's share of national production (NASS 2022 Census of
    Agriculture) — and both stay interactive in the page. `apps/site/test/claims.test.ts` asserts every count
    on the page against the config file it came from, so a marketing number cannot drift from the model.

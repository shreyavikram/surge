...
describe('gemini proposer', () => {
  ...
  it('parses a mocked Gemini response', async () => {
    const payload = { candidates: [{ content: { parts: [{ text: JSON.stringify([{ name: 'India rice export ban', category: 'export_ban', regionId: 'india', commodities: ['rice'], severity: 0.8, months: 6, confidence: 0.95, matched: ['bans', 'rice', 'exports'] }]) }] } }] };
    ...
    expect(out[0]!.regionId).toBe('india');
  });
  it('merges LLM and rule candidates without duplicating a category+region', () => {
    ...
    const llm = proposalsToCandidates([{ name: 'x', category: 'export_ban', regionId: 'india', commodities: ['rice'], severity: 0.8, months: 6, confidence: 0.9 }], ctx);
    const merged = mergeCandidates(llm, rules);
    expect(merged.filter((c) => c.category === 'export_ban' && c.regionId === 'india')).toHaveLength(1);
